...
export default function App() {
  ...
  useEffect(() => {
    ...
    let stopSurveySnapshot: (() => void) | null = null;
    ...
    return () => {
      ...
      if (stopSurveySnapshot) stopSurveySnapshot();
    };
  }, []);
  ...
  useEffect(() => {
    if (user && selectedUnit && !selectedUnit.id.startsWith('unit-')) {
      ...
      const stopSurveySnapshot = onSnapshot(q, (snapshot) => {
        setUnitSurveys(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as TechnicalSurvey)));
      });

      return () => stopSurveySnapshot();
    } else {
      setUnitSurveys([]);
    }
  }, [user, selectedUnit]);
            ...
            <section className="grid grid-cols-1 xl:grid-cols-2 gap-6 pb-8">
              ...
              <GlassCard>
                <div className="flex items-center justify-between mb-5">
                  ...
                  <button 
                    onClick={() => setActiveTab('surveys')}
                    className="text-[9px] font-mono text-emerald-400 hover:underline uppercase tracking-widest"
                  >
                    Abrir Auditoria
                  </button>
                </div>
